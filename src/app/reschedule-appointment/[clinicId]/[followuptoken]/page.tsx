...
export default function RescheduleAppointmentPage() {
  ...
  // Load appointment details and related data
  useEffect(() => {
    const loadAppointmentData = async () => {
      try {
        ...
        const appointmentResponse = await patientService.getRescheduleAppointmentDetails(clinicId, followupToken);
        ...
        // Load providers list
        const providersResponse = await patientService.getProvidersList();
        if (providersResponse.success && providersResponse.data) {
          setProviders(providersResponse.data);
        }

        // Load available dates and time slots (mock for now - replace with real API calls)
        setAvailableDates([
  ...
  const { date: currentDate, time: currentTime } = formatDateTime(appointmentData.appointment.scheduled_for);
